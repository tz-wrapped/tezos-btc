...
import Client.Types
import Lorentz (TSignature(..), toAddress, toTAddress)
...
import Morley.Tezos.Address.Alias (AddressOrAlias(..), SomeAddressOrAlias(..), unAlias)
...
defaultHandlers = Handlers
  ...
  , hGetAliasesAndAddresses = unavailable "getAliasesAndAddresses"
  }
  where
...
multiSigOverrideAddress :: ContractAddress
multiSigOverrideAddress = [ta|KT1MwaBC3G3cUa3PfjJ1StFkSuBLbRuoReRK|]

operatorAddress1 :: ImplicitAddress
    ...
    where
      ...
      checkToSign package = case getToSign package of
        Right ((chainId, addr), (Counter counter, _)) -> pure $
          ( addr == toAddress multiSigAddress && chainId == testChainId &&
            counter == 14 )
      ...
      checkPackage bs = case decodePackage bs of
        Right package -> case fetchSrcParam package of
          Right param -> do
            ...
            pure $ toSignOk && param `isAddOperatorParam` operatorAddress1
...
test_createMultisigPackage = testGroup "Create multisig package"
  ...
  where
    mockArgs = CmdAddOperator (SAOAKindSpecified $ AddressResolved operatorAddress1) (Just multiSigFilePath)
...
multiSigCreationWithMSigOverrideTestHandlers =
  defaultHandlers
    ...
    where
      ...
      checkToSign package = case getToSign package of
        Right ((chainId, addr), (Counter counter, _)) -> pure $
          ( addr == toAddress multiSigOverrideAddress && chainId == testChainId &&
            counter == 14 )
        _ -> throwM $ TestError "Getting address and counter from package failed"
      checkPackage bs = case decodePackage bs of
        Right package -> case fetchSrcParam package of
...
isAddOperatorParam :: TZBTCTypes.Parameter ver -> ImplicitAddress -> Bool
isAddOperatorParam param addr = case param of
  TZBTCTypes.SafeEntrypoints (TZBTCTypes.AddOperator a)
    -> a == (#operator :! toAddress addr)
  _ -> False
...
multisigSignPackageTestPackage = mkPackage
  ...
  (TZBTCTypes.AddOperator (#operator :! toAddress operatorAddress1))
